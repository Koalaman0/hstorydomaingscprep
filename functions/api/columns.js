...
import { convertColumn } from "../_lib/convert.js";
import { getColumnBySlug, upsertColumn, deleteColumn } from "../_lib/db.js";
...
export async function onRequestPost({ request, env }) {
  ...
  const existing = await getColumnBySlug(env, origSlug || raw.slug);
  const existingBySlug = new Map(existing ? [[existing.slug, existing]] : []);
  const converted = convertColumn(raw, existingBySlug);

  try {
    await upsertColumn(env, converted, origSlug);
  } catch (e) {
...
export async function onRequestDelete({ request, env }) {
  ...
  await deleteColumn(env, payload.slug);
  return new Response(JSON.stringify({ ok: true }), { status: 200, headers: { "Content-Type": "application/json" } });
}
