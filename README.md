...
- 정적 HTML/CSS/JS로 만든 정보 사이트 (홈, 카테고리 5개, 글 15개, 칼럼 3개, 신뢰 페이지 6종, 사이트맵, 404)
- `/admin/` 경로의 관리자 화면 — 글/칼럼/카테고리 작성·수정, 사이트 설정(히어로 배경 이미지 등) 편집
- `/login/` 관리자 전용 로그인 페이지 (메인 헤더의 "로그인" 버튼으로 접근)
- `/functions/` 아래 Cloudflare Pages Functions — 실제 서버 사이드 로그인 인증과, 관리자가 저장한 내용을
  GitHub 저장소에 직접 커밋하는 API. 정책 페이지(개인정보처리방침/이용약관/면책고지)는 이 API가 다루는
  `data/content.json`에 포함되지 않으므로 관리자 화면에서 수정할 수 없습니다.

## 배포 방법 (Cloudflare Pages, 무료 티어)

1. https://dash.cloudflare.com 에서 **Workers & Pages → Create → Pages → Connect to Git**로 이 저장소를 연결합니다.
2. 빌드 설정:
   - **Build command**: `python3 scripts/build.py`
   - **Build output directory**: `/` (저장소 루트)
3. **Settings → Environment variables**에서 아래 값들을 **Secret**으로 등록합니다.

   | 변수명 | 설명 |
   |---|---|
   | `ADMIN_PASSWORD` | 관리자 로그인 비밀번호(평문 그대로 저장 — Cloudflare가 암호화해서 보관합니다) |
   | `SESSION_SECRET` | 로그인 세션 쿠키 서명에 쓰는 임의의 긴 무작위 문자열 (예: `openssl rand -hex 32`로 생성) |
   | `GITHUB_TOKEN` | 이 저장소의 `contents` 쓰기 권한을 가진 GitHub Fine-grained Personal Access Token |
   | `GITHUB_OWNER` | 저장소 소유자 (예: `Koalaman0`) |
   | `GITHUB_REPO` | 저장소 이름 (예: `hstory`) |
   | `GITHUB_BRANCH` | Cloudflare Pages가 배포에 사용하는 브랜치명 (예: `main`) |

4. 커스텀 도메인은 **Custom domains** 탭에서 구매한 도메인을 연결하면 됩니다(무료).
5. 저장이 완료되면 관리자 화면(`/admin/`)에서 "지금 사이트에 게시"를 누를 때마다 `GITHUB_TOKEN`으로
   `data/content.json`을 직접 커밋하고, 그 push가 Cloudflare Pages의 자동 재배포(빌드 커맨드 재실행)를
   트리거합니다. 보통 1분 안에 실제 사이트에 반영됩니다.
...
3. **일반 글 관리 / 칼럼 관리 / 카테고리 / 사이트 설정**에서 내용을 작성·수정합니다. 이 화면에서의 "저장"은
   우선 이 브라우저에만 임시 보관되며, **사이트 설정 화면의 "지금 사이트에 게시"**를 눌러야 실제로 반영됩니다.
   - 본문 입력란에는 소제목 앞에 `## `을 붙이고, 문단 사이는 빈 줄로 구분해 주세요.
   - 사이트 설정의 **히어로 배경 이미지 URL**에 이미지 주소를 입력하면 메인 페이지 히어로 섹션 배경이 바뀝니다.
   - 개인정보처리방침/이용약관/면책고지 페이지는 관리자 화면에서 다루지 않는 별도 정적 페이지입니다.
4. "지금 사이트에 게시"를 누르면 `/functions/api/content.js`가 로그인 여부를 다시 확인한 뒤, GitHub API로
   `data/content.json`을 커밋합니다. 이 push가 Cloudflare Pages 재배포를 트리거해 사이트에 반영됩니다.

### 알아두면 좋은 한계

- 관리자 계정은 비밀번호 1개로 동작하는 단일 관리자용 로그인입니다(회원가입/권한 구분 없음).
- 로그인 실패 횟수 제한(rate limit)은 구현되어 있지 않으므로, 충분히 길고 추측하기 어려운 비밀번호를 사용하세요.
- 이미지 업로드 기능은 없고, 외부에 호스팅된 이미지의 URL을 입력하는 방식입니다.

## 로컬에서 콘텐츠만 수정하고 싶을 때

`scripts/build.py`를 로컬에서 직접 실행해도 됩니다(Cloudflare Pages 없이 정적 파일만 다시 만들고 싶을 때).
...
data/content.json          ← 사이트의 유일한 콘텐츠 소스 (설정/카테고리/글/칼럼)
        ...
        └─ functions/api/content.js          admin "게시" 요청을 받아 GitHub에 직접 커밋 (Cloudflare Pages)
```
...
├─ posts/<slug>/               글 상세 15개
├─ columns/<slug>/             칼럼 상세 3개 (+ /columns/ 목록)
├─ admin/                      관리자 화면
...
├─ assets/js/admin.js          관리자 화면 로직 + 게시 API 호출
├─ assets/icons/favicon.svg
├─ functions/
│  ├─ api/login.js             비밀번호 검증 + 세션 쿠키 발급
│  ├─ api/logout.js            세션 쿠키 삭제
│  ├─ api/session.js           로그인 여부 확인
│  ├─ api/content.js           게시 요청 → GitHub에 content.json 커밋
...
| 관리자 비밀번호 / 세션 설정 | Cloudflare Pages의 환경변수(Secret) `ADMIN_PASSWORD` / `SESSION_SECRET` |
...
- 관리자 인증/게시 기능은 Cloudflare Pages Functions 환경변수가 설정되어 있어야 동작합니다(위 "배포 방법" 참고). 설정 전에는 로그인이 항상 실패합니다.
